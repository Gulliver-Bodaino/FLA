...
# $to1  = 'tahara@ww-system.com';    # 管理者アドレス
# $from = 'tahara@ww-system.com';    # from用管理者アドレス

$to1  = 'fujimori@gulliver.jp';    # 管理者アドレス
$from = 'fujimori@gulliver.jp';    # from用管理者アドレス
# $to1 = 'kubota@gulliver.co.jp';
# $from = 'kubota@gulliver.co.jp';
# $to1 = 'terajima@flanet.jp';
# $from = 'terajima@flanet.jp';

# メールタイトル
$subject  = '食生活アドバイザー検定　登録情報変更完了';    # 管理者用
...
sub fileinput {
    ...
    # バックアップ
    mkdirs($backup_dir);
    copy qq|$csv_dir/$csv_data|, qq|$backup_dir/backup$csv_data|;
...
sub maildata1 {
    ...
    #     open( MAIL, "| /usr/sbin/sendmail -t" ) || die "send error please once again\n";
    #     print MAIL "TO: $to1\n";
...
<!--個人-->
		...
		<tr>
			<td bgcolor="#d2de84"><font class="t12">マンション</font></td>
...
<font class="t12">〒<input type="hidden" name="shinadd1" value="$shinadd1">$shinadd1-<input type="hidden" name="shinadd2" value="$shinadd2">$shinadd2　</font>
		...
		<tr>
			<td bgcolor="#fdc0b5"><font class="t12">マンション</font></td>
...
sub checkdata {
    ...
    if ( $shinmailcheck ne "" ) {
        if ( ( $shinmail1 eq "" ) && ( $shinmail2 eq "" ) && ( $shinmail3 eq "" ) && ( $shinmail4 eq "" ) ) {
    ...
    if ( $shinmail1 ne "" ) {
        if ( $shinmail1 =~ /^[\w\-\.]+$/ ) {
            ...
            $checkshinmail = "[メールアドレス変更項目のメールアドレス(確認用も含む)]が正しく入力されていません。";
            $checkkazu     = "1";
        }
    }
    if ( $shinmail2 ne "" ) {
